...
class Sequential(Module):
    ...
    def location(self, LRP, keep_batch=False):
        ...
        k=(l//7)
        if args.interpreter == 'lrp' and args.lrp_target_layer == None:
            LRP = LRP / LRP.max()
    ...
    def interpretation(self, activation_output, interpreter, labels, target_layer = args.lrp_target_layer, inputs=None):
        if inputs is None and interpreter in ['smooth_grad', 'integrated_grad', 'smooth_grad_T', 'integrated_grad_T']:
            print('you need to give inputs!!', tt)
        
        if interpreter == 'lrp':
            LRP = self.lrp(activation_output, labels=labels, lrp_var=args.r_method, param=1e-8, whichScore=None, target_layer = None)
            
        elif interpreter == 'lrp_T':
            LRP = self.lrp(activation_output, labels=labels, lrp_var=args.r_method, param=1e-8, whichScore=None, target_layer = target_layer)
            
        elif interpreter == 'grad_cam':
            LRP = self.grad_cam(activation_output, labels, target_layer)
                
        elif interpreter == 'simple_grad':
            LRP = self.simple_grad(activation_output, labels, None)
    ...
    def integrated_grad(self, net, inputs, labels, target_layer, by_label = True, num_target = 1000):
        ...
        for i in range(iterations):
            ...
            if i == 0:
                R = self.simple_grad(activation_output, labels, target_layer, integrated_grad=True).detach()
            else:
    ...
    def grad_cam(self, output, labels, target_layer, by_label = True, num_target = 1000):
        eye = torch.eye(num_target, dtype=torch.float32)
        eye = eye.cuda()
        if by_label:
            dx = eye[labels]
    ...
    def lrp(self, R, labels=None, lrp_var=args.r_method, param=None, whichScore=None, target_layer=None):
        '''
